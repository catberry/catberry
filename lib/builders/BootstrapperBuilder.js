...
class BootstrapperBuilder {
	...
	build(stores, components) {
		...
		this._logger.info('Building bootstrapper...');
		...
		return pfs.readFile(bootstrapperTemplatePath, {
			...
			.then(context => pfs.exists(routeDefinitionsPath))
			.then(isExists => {
			...
			.then(boostrapper => {
				const timeMessage = hrTimeHelper.toMessage(hrTimeHelper.get(startTime));
				this._logger.info(`Bootstrapper has been built (${timeMessage})`);
				return boostrapper;
			})
			...
			.forEach(componentName => {
				...
				const promise = this._compileComponentTemplates(componentDetails)
					.then(templates => {
						...
						const templatesString = templates.length > 1 &&
						...
						return `
{
	...
	errorTemplateSource: ${templatesString}
}`;
					});
	...
	_compileComponentTemplates(componentDetails) {
		return Promise.all(this._getTemplates(componentDetails))
			.then(templates => {
