...
class BrowserBundleBuilder {
	...
	_createBundler() {
		...
		if (!this._isRelease) {
			...
			this._logger.info('Watching files for changes to rebuild bundle...');
		} else {
			this._bundler.transform(file => {
				if (path.extname(file) !== '.js') {
					return new stream.PassThrough();
				}
				this._logger.trace(`Minifying code of file "${file}"...`);
		...
		var startTime;
		const resetHandler = () => {
			this._logger.info('Building browser script bundle...');
			startTime = hrTimeHelper.get();
		};

		this._bundler
			.on('update', ids => {
				this._logger.info(`Bundle has been updated, changed files: [${ids.join(',')}]`);
		...
		return Promise.resolve()
			.then(() => {
				...
				if (!actionObject ||
					...
					this._logger.warn('The post-build action has an incorrect interface, skipping...');
					return null;
				}
	...
	_setTransformations() {
		...
		while (currentIndex >= 0) {
			...
			if (!currentTransformation ||
				...
				this._logger.warn('The browserify transformation has an incorrect interface, skipping...');
				continue;
			}
	...
	_setPlugins() {
		...
		while (currentIndex >= 0) {
			...
			if (!currentPlugin ||
				...
				this._logger.warn('The browserify plugin has an incorrect interface, skipping...');
				continue;
			}
