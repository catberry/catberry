...
class ComponentFinder extends events.EventEmitter {
	 ...
	 *
	 * @return {Promise} Promise resolve when all watcher will be ready
	 */
	watch() {
		if (this._readyWatchers) {
			return this._readyWatchers;
		}

		const readyDefers = [Promise.defer(), Promise.defer()];

		this._readyWatchers = Promise
			.all([readyDefers[0].promise, readyDefers[1].promise])
			.then(data => {
		...
		fileWatcher.on('ready', () => readyDefers[0].resolve(fileWatcher))
		...
		componentJsonWatcher.on('ready', () => readyDefers[1].resolve(componentJsonWatcher))
			.on('error', error => this._eventBus.emit('error', error))
			// add new component
			.on('add', filename => {
	 ...
	 * Close watchers
	 *
	 * @return {Promise} Promise resolved when all watcher will be close
	 */
	closeWatch() {
		if (this._readyWatchers) {
			const promise = this._readyWatchers;

			this._readyWatchers = null;

			return promise.then(watchers => watchers.map(watcher => watcher.close()));
		}
