...
class ComponentFinder extends events.EventEmitter {
	...
	constructor(locator) {
		...
		} else if (componentsGlob instanceof Array) {
	...
	find() {
		...
		this._foundComponentsByNames = new Map();
		this._foundComponentsByDirs = new Map();
	 ...
	 * @returns {{name: string, path: string, properties: Object}?} The found
	...
	_recognizeComponent(filename) {
		...
		while (current !== '.') {
			if (this._foundComponentsByDirs.has(current)) {
				component = this._foundComponentsByDirs.get(current);
				break;
			}
	 ...
	 * @param {{name: string, path: string, properties: Object}?} The found
	...
	_addComponent(component) {
		...
		if (this._foundComponentsByNames.has(component.name)) {
			const existedComponent = this._foundComponentsByNames.get(component.name);
			this._logger.warn(
		...
		const dirName = path.dirname(component.path);
		this._foundComponentsByNames.set(component.name, component);
		this._foundComponentsByDirs.set(dirName, component);
