...
module.exports = {
	 ...
	 * @returns {string} The URI without slash at the end.
	 */
		...
		if (routeUri.query) {
			...
			Object.keys(routeUri.query.values)
				.forEach(name => {
					...
					if (routeUri.query.values[name] instanceof Array) {
						return;
					}
	...
	return (queryValues, state) => {
		...
		Object.keys(queryValues)
			.forEach(queryKey => {
				...
				const value = queryValues[queryKey] instanceof Array ?
				...
				parameter.storeNames.forEach(storeName => {
					if (storeName[state] === null ||
						typeof (storeName[state]) !== 'object') {
						state[storeName] = Object.create(null);
					}
					state[storeName][parameter.name] = value;
				});
