...
class ComponentReadable extends stream.Readable {
	...
	_foundComponentHandler(tagDetails) {
		...
		const id = tagDetails.attributes[moduleHelper.ATTRIBUTE_ID];
		...
		if (isDocument) {
			...
			this._context.eventBus.emit('warn', `Component "${componentName}" does not have an ID, skipping...`);
			return null;
		} else if (this._context.renderedIds[id]) {
			this._context.eventBus.emit('warn',
	...
	_handleComponentError(context, error) {
		...
		if (!isRelease && error instanceof Error &&
			...
			try {
				return errorHelper.prettyPrint(error, context.userAgent);
			} catch (e) {
				return '';
			}
		}
