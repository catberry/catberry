...
class ModuleApiProvider extends ModuleApiProviderBase {
	...
	/**
	 * Redirects current page with 302 HTTP code status to the specified URI.
	 * @param {string} uriString The URI to direct.
	 * @returns {Promise} The promise for finished work.
	 */
	redirect(uriString) {
		this.actions.redirectedTo = uriString;
		this.actions.codeStatus = 302;
		return Promise.resolve();
	}

	/**
	 * Redirects current page with 301 HTTP code status to the specified URI.
	 * @param {string} uriString The URI to direct.
	 * @returns {Promise} The promise for finished work.
	 */
	permRedirect(uriString) {
		this.actions.redirectedTo = uriString;
		this.actions.codeStatus = 301;
		return Promise.resolve();
	}
