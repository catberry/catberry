...
const PrettyPrintWritable = require('../streams/PrettyPrintWritable');

const DEFAULT_LOGGER_CONFIG = {
	name: 'catberry',
	stream: new PrettyPrintWritable()
};
...
class BootstrapperBase {
	...
	configure(configObject, locator) {
		...
		const logger = bunyan.createLogger(configObject.logger || DEFAULT_LOGGER_CONFIG);
		locator.registerInstance('bunyan', bunyan);
	...
	wrapEventsWithLogger(config, eventBus, logger) {
		...
		eventBus.on('error', error => logger.error(error));
