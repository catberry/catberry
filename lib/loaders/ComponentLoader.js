	...
	load() {
		...
		return this._componentFinder.find()
			...
			.then(componentList => {
				...
				if (!this._isRelease) {
					this._logger.info('Watching components for changes...');
	...
	_handleChanges() {
		...
		this._componentFinder
			.on('add', componentDetails => {
				this._logger.info(`Component "${componentDetails.path}" has been added, initializing...`);
			...
			.on('changeLogic', componentDetails => {
				this._logger.info(`Scripts of the "${componentDetails.path}" component have been changed, reinitializing...`);
				requireHelper.clearCacheKey(this._getLogicPath(componentDetails));
				loadComponent(componentDetails);
			})
			.on('changeTemplates', componentDetails => {
				this._logger.info(`Template of the "${componentDetails.path}" component have been changed, reinitializing...`);
				loadComponent(componentDetails);
			})
			.on('unlink', componentDetails => {
				this._logger.info(`Component "${componentDetails.path}" has been unlinked, removing...`);
				requireHelper.clearCacheKey(this._getLogicPath(componentDetails));
